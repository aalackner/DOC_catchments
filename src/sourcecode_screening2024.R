...
add_individual_trend <- function(x,plot, y=NULL, title=NULL){
  ...
  plot + geom_line(aes(x = date, y=trend, color=as_factor(signif_sign), group=c(0)), lwd=1.5, data = out)->p
  return(p)
}
...
modeling_multi <- function(x, link = "identity", autocor = autocor, tdist = tdist) {
  # family <- if(log == TRUE){gaussian(link="log")}else{gaussian()}
  formula <- variable ~ s(decimaldate, k = round(nrow(x)/2)) + s(decimal_during, bs = "cc", k = 13) + s(abs_f_420_5cm) + s(n_comb) + s(cl_mg_l) + s(alk_acid_mekv_l) + s(so4_komb) + s(p15) + s(q15_log) + s(t15)
  knots <- list(decimal_during = seq(0, 1, length=13))
  x <- drop_na(x, variable)
  if(autocor == TRUE)
  {out <- try(model(x, link, formula, knots))
...
screeningmodeling_multi <- function(.data,
                                  datevar, #variabel med datum (i datumformat!)
                                  values, # variabel med värden
                                  expl_variable.type = "linear",
                                  link = "identity",
  ...
  .data %>%
    mutate(variable = !!variable,
           date = !!datevar) %>%
    select(date, variable,q15_log, t15, p15, so4_komb, alk_acid_mekv_l, n_comb, cl_mg_l,abs_f_420_5cm, !!!nestvars) %>%
    group_by(!!!nestvars, date) %>%
    summarise_all(mean) %>%
    ungroup() %>%
    drop_na() %>%
    # drop_na(variable) %>%
    # drop_na(any_of(c(q15_log, t15, p15, so4_komb, alk_acid_mekv_l, n_comb, cl_mg_l,abs_f_420_5cm))) %>%
    group_by(!!!nestvars) %>%
    #  complete(date = full_seq(date, 1)) %>%
    mutate(decimaldate = decimal_date(date),
           month = month(date),
    ...
    ungroup() %>%
    mutate(
      fit = future_map(data, possibly(~ modeling_multi(.x,
                                                      link = link,
                                                      autocor = autocor, tdist = tdist),
                                      otherwise = NA_integer_,
                                      quiet = F),
