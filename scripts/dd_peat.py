...
def plot_peat_catchment(polygon, peat_gt1, lines_clip, lines_in_peat, perc, args_o, idx=None):
    ...
    Parameters:
    - polygon: geopandas GeoSeries or GeoDataFrame row (with geometry and 'mvm_id' key)
    - peat_gt1: xarray DataArray with peat raster (values 0-4)
    - lines_clip: GeoDataFrame with lines clipped outside peat
    - lines_in_peat: GeoDataFrame with lines inside peat
    - perc: float, percentage of lines in peat (for title)
    - args_o: string, output filepath (used to create 'maps' folder)
    - idx: optional int or str for polygon index if 'mvm_id' not present
    """
    ...
    # Plot peat raster with discrete colormap
    ...
    # Plot clipped lines inside peat and outside
    ...
    plt.title(f"Catchment {polygon['mvm_id'] if 'mvm_id' in polygon else idx} with {perc:.0f}% in peat")
    plt.legend(loc='upper right')
    plt.xlabel("X (meters)")
    plt.ylabel("Y (meters)")

    # Shrink and center colorbar vertically
    ...
    filename = f"peat_{polygon['mvm_id'] if 'mvm_id' in polygon else idx}.jpeg"
...
def process_catchment_v2(polygon, peat, lines, args_o, idx=None):
    ...
    try:
        print("Calculating pixel percentages per peat class inside polygon...")
        peat_data = peat_masked.data[0]  # assuming single band raster
        peat_data_flat = peat_data.compressed() if hasattr(peat_data, 'compressed') else peat_data.flatten()
        peat_data_flat = peat_data_flat[~np.isnan(peat_data_flat)]
        total_pixels = len(peat_data_flat)
    ...
    try:
        ...
        plot_peat_catchment(polygon, peat_gt1, lines_clip, lines_in_peat, results['perc_lines_in_peat'], args_o, idx)
    except Exception as e:
...
# Paths
gdb_path = r"C:\Users\anlr0006\repos-win\DOC_catchments\input\Dikeskarta\mosaic_ditches.gdb"
...
# Load data
polygons = gpd.read_file(f"zip://{polygon_fp}").to_crs("EPSG:3006").iloc[0:3]
lines = gpd.read_file(gdb_path, layer=layer_name).to_crs("EPSG:3006")
peat = rioxarray.open_rasterio(peat_raster_fp, masked=True).rio.write_crs("EPSG:3006")
