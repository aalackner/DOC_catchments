...
input_file = "input/SMHI/SVARO_discharge.csv"
removed_file = 'results/SVARO/S-HYPE_uncertainty.txt'
remaining_file = 'results/SVARO/SVARO_discharge.csv'
...
svaro_discharge = pd.read_csv("results/SVARO/SVARO_discharge.csv", sep = ";", decimal= ",", parse_dates= [2] )
svaro_discharge
#%%
...
discharge = pd.DataFrame(columns=['mvm_id', 'Datum', 'q', 'Shape_Area'])
mvm_ids = gdf['mvm_id'].unique()
for id in mvm_ids:
    aroid = gdf.loc[gdf['mvm_id']== id]['ARO_UUID'].values[0]
    lokal_area = gdf.loc[gdf['mvm_id']== id]['AREA'].values[0]
    area = gdf.loc[gdf['mvm_id']== id]['Shape_Area'].values[0]
    local_q = svaro_discharge.loc[svaro_discharge['Aroid'] == aroid]
    local_q['mvm_id'] = id
    local_q['Shape_Area'] = area
    weight = gdf.loc[gdf['mvm_id']== id]['weight'].values[0]
    # lokal_weight = gdf.loc[gdf['mvm_id']== id]['weight_lokal'].values[0]
    if (lokal_area > area) & ( weight < 0.1) :
        # print(id)
        weight = gdf.loc[gdf['mvm_id']== id]['weight_lokal'].values[0]
    ...
    discharge = pd.concat([discharge, local_q[['mvm_id', 'Datum','q']]])
discharge.describe()
# %%
...
discharge.to_csv("results/discharge/daily_discharge.csv", index = False)
